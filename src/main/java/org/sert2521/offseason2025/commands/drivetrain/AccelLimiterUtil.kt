...
object AccelLimiterUtil {
    ...
    fun readJoysticks(
         ...
         */
        x = Input.getJoystickX()
        y = Input.getJoystickY()
        ...
        newMagnitude = sqrt(
            MathUtil.applyDeadband(sqrMagnitude, ControllerConstants.POWER_DEADBAND.pow(2))
        ).pow(3)
        ...
        if (fieldOriented) {
            curvedChassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(
                ...
                MathUtil.applyDeadband(Input.getJoystickZ(), ControllerConstants.ROTATION_DEADBAND)
